<?php
/*
Plugin Name: ElPuas Popup
Plugin URI: http://github.com/elpuas/elpuas-popup
Description: Shortcode to open a link inside a popup browser window
Author: 3LPU4S
Version: 1.3.0
Author URI: http://elpuas.com/
License: GPLv3

  Copyright 2016 Alfredo Navas
...
function ep_popup_plugin_version() {
...
add_filter( 'plugin_row_meta', 'eppopup_meta_links', 10, 2 );
function ep_popup_meta_links( $links, $file ) {
...
function ep_popup_script() {
	wp_register_script( 'popup', plugins_url() . "/" . basename(dirname(__FILE__)) . '/js/popup.js', array('jquery'), ep_popup_plugin_version(), false );
	wp_enqueue_script( 'popup' );
 }
 
add_action('wp_enqueue_scripts', 'eppopup_script');

// ==============================================
// shortcode to add a link with the 'popup' class with data attributes for height, width and scrollbars
// ==============================================

function ep_popup_shortcode( $atts, $content = null ) {
	...
	return '<a href="' . esc_url( $url ) . '" class="elpuas-popup" data-width="' . absint( $width ) . '" data-height="' . absint( $height ) . '" data-scrollbars="' . $showscrollbars . '" alt="' . esc_attr( $alt ) . '">' . $content . '</a>';
}

add_shortcode( 'popup', 'eppopup_shortcode' );
